...
with open('result.csv','w',encoding="utf-8") as f:
        f.write("Name , Date ,Year,Review \n")
chrome_path=r"C:\Users\Medasani Chaithanya\Desktop\chromedriver.exe"
...
for i in range(1,11):
    ...
    time.sleep(5)
    posts_review=driver.find_elements_by_class_name("readable")
    time.sleep(5)
    
    with open('result.csv','a',encoding="utf-8") as f:
            for j in range(1,len(posts)):
                    f.write(posts[j].text+ "," + posts_date[j].text+","+posts_review[j].text+"\n")
    time.sleep(10)
